...
public class RemoteFileUtil {
    ...
    /**
     * Decode the Throwable and rethrow a new RuntimeException or IllegalArgumentException
     * 
     * @param throwable	the throwable to analyse
     * @throws IllegalArgumentException	if the filter java version is > host java version
     * @throws RuntimeException the RuntimeException to rethrow
     */
    public static void decodeTrowableForFilterUsage(Throwable throwable)
        ...
        if (throwable instanceof RemoteException) {
        	...
        	if (message != null
        	    ...
        	    String classFileVersion = RemoteFileUtil.decodeJavaVersionFromMajorMinor(classFileVersionNumber);
     ...
     * @return
     */
    public static String decodeJavaVersionFromMajorMinor(String classFileVersionNumber) {
        //6 (1.6)  50.0
        //7 (1.7)  51.0
        //8 (1.8)  52.0
     ...
     * 
     * @param base64SerialFilter	the serialized filter
     * @param remoteSession 		the remote session to use for upload
     ...
     */
    public static String uploadFilterIfShortSize(String base64SerialFilter, RemoteSession remoteSession)
            throws IOException, IllegalArgumentException,
            InvalidLoginException, FileNotFoundException, UnknownHostException,
            ConnectException, SocketException, RemoteException {
        ...
        if (! RemoteFileUtil.isFilterShortSize(base64SerialFilter)) {
            ...
            try {
		FileUtils.writeStringToFile(file, base64SerialFilter, Charset.defaultCharset());
		...
		try {
		    ...
		    out = new RemoteOutputStream(remoteSession, filterFilename, file.length());
		    IOUtils.copy(in, out);
     ...
     * @param filenameFilter		the FilenameFilter to serialize
     * @param filenameFilterClassname	the FilenameFilter class name (for Exception)
     * @return	the serialized FilenameFilter in base64 String
     * 
     * @throws IOException 
     */
    public static String SerializeBase64FilenameFilter(FilenameFilter filenameFilter,
            String filenameFilterClassname) throws IOException {
    
        if (!(filenameFilter instanceof Serializable)) {
            throw new IllegalArgumentException(Tag.PRODUCT
        	    + " the FilenameFilter is not serializable: "
     ...
     * @param filenameFilter		the FileFilter to serialize
     * @param filenameFilterClassname	the FileFilter class name (for Exception)
    ...
    public static String SerializeBase64FileFilter(FileFilter fileFilter,
        ...
        if (!(fileFilter instanceof Serializable)) {
            throw new IllegalArgumentException(Tag.PRODUCT
        	    + " the FileFilter is not serializable: "
        	    + fileFilterClassname);
        }
    ...
    /**
     * Test is filter is short size, in ordr to decide to send it in request or per file upload
     * @param base64SerialFilter	the serialized base64 filter
    ...
    public static boolean isFilterShortSize(String base64SerialFilter) {
        ...
        if (base64SerialFilter.length() <= Settings.MAX_STRING_SIZE_FOR_HTTP_REQUEST) {
            return true;
        }
        else {
            return false;
